...
public class FirstAvailabilityParkingBoyTest extends BaseParkingBoyTest {
    ...
    public void should_print_status() {
        assertThat(role.getStatus()).isEqualTo("[ParkingBoy: 2 total, 0 occupied, 2 available\n"
                + "\t[ParkingLot: 1 total, 0 occupied, 1 available],\n"
                + "\t[ParkingLot: 1 total, 0 occupied, 1 available]]");
        role.park(car);
        assertThat(role.getStatus()).isEqualTo("[ParkingBoy: 2 total, 1 occupied, 1 available\n"
